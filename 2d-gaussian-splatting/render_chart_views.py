...
if __name__ == "__main__":
    ...
    charts_depth_normals = get_surf_normal_parallel(train_viewpoints, charts_depths)
    charts_prior_depths = charts_priors['prior_depths']
    charts_confs = charts_priors['confs']
    charts_mono_normals = charts_priors['normals']
    ...
    for idx in range(len(visibility_times_masks)):
        ...
        if rgb_image.shape[:2] != match_mask.shape:
            rgb_image = cv2.resize(rgb_image, (match_mask.shape[1], match_mask.shape[0]), interpolation=cv2.INTER_LINEAR)
