...
def downsample_pointcloud(points, voxel_size=0.02):
    points_np = points.detach().cpu().numpy().astype(np.float64)
    N = points_np.shape[0]
    if N == 0:
        return points
    min_coords = points_np.min(axis=0)
    points_shifted = points_np - min_coords
    voxel_indices = np.floor(points_shifted / voxel_size).astype(np.int32)
    
    unique_indices, group_indices = np.unique(voxel_indices, axis=0, return_inverse=True)
    M = unique_indices.shape[0]
    downsampled_np = np.zeros((M, 3), dtype=np.float64)
    for i in range(M):
        downsampled_np[i] = points_np[group_indices == i].mean(axis=0)
    
    downsampled = torch.from_numpy(downsampled_np).to(points.device).float()
    return downsampled
...
def vis_plane_mask(plane_mask, plane_id, rgb_path, save_path):
    ...
    mask_h, mask_w = obj_mask_map.shape
    if rgb.size != (mask_w, mask_h):
        rgb = rgb.resize((mask_w, mask_h), Image.BILINEAR)
    rgb = np.array(rgb)
...
def get_plane_pnts_idx_from_mask(pixel_pnts_map, plane_mask, plane_id):
    ...
    plane_pnts_idx = plane_pnts_idx_temp.unique()
    plane_pnts_idx = plane_pnts_idx[plane_pnts_idx != 0]
