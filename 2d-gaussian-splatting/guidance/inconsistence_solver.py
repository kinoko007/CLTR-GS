...
if __name__ == "__main__":
    ...
    assert len(pnts_list) == len(train_viewpoints)
    ...
    assert len(refine_depth_list) == len(train_viewpoints)
    ...
    for view_idx, confident_map in confident_maps.items():
        ...
        H_conf, W_conf = confident_map_np.shape[:2]
        if rgb_image.size != (W_conf, H_conf):
            rgb_image = rgb_image.resize((W_conf, H_conf), Image.LANCZOS)
