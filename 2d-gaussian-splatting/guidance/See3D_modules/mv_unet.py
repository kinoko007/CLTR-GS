...
class BasicTransformerBlock3D(nn.Module):
    ...
    def forward(self, x, context=None, num_frames=1):# torch.Size([8, 1024, 320])
        x = rearrange(x, "(b f) l c -> b (f l) c", f=num_frames).contiguous()# torch.Size([2, 4096, 320]) [b, t*h*w, c]
...
class MultiViewUNetModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        self,
        x,# torch.Size([8, 4, 32, 32]) [n, c, h, w]
        timesteps=None,# torch.Size([8])
        context=None,# torch.Size([8, 77, 1024])
        y=None,
        camera=None,# torch.Size([8, 16])
        num_frames=1,# 4
        ip=None,
        ip_img=None,
        **kwargs,
